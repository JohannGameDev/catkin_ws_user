...
def callbackBackwardLongitudinal(request):
    ...
    rospy.sleep(7)
    driving_direction_pub2.publish(0.5)

    rospy.sleep(7)
    pub_forward.publish(0.5)

    #rospy.sleep(10)
    return ParkingManeuverResponse("fertig 3")


###
def ausparken(request):
    rospy.loginfo(rospy.get_caller_id() + ": ausparken, direction = " + request.direction)

    if request.direction == "right":
        driving_direction_pub1 = pub_forward_left
        driving_direction_pub2 = pub_forward_right
    elif request.direction == "left":
        driving_direction_pub1 = pub_forward_right
        driving_direction_pub2 = pub_forward_left
    else:
        return ParkingManeuverResponse(
            "ERROR: Request can only be 'left' or 'right'")

#    rospy.sleep(10)
    pub_back.publish(0.5)
    ...
    #rospy.sleep(10)
    return ParkingManeuverResponse("fertig ausgeparkt")
...
###
sub_ausparken = rospy.Service(
    "simple_parking_maneuver/ausparken",
    ParkingManeuver,
    ausparken)
###
